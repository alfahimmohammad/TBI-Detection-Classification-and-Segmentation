...
def window_ct (ct_scan, w_level=40, w_width=80):
    ...
    for s in range(num_slices):
        slice_s = ct_scan[:,:,s]
        slices = np.clip(slice_s,w_min,w_max)
        mini = np.min(slices)
        maxi = np.max(slices)
        slices = (slices - mini)*(255/(maxi-mini)) #or slice_s = (slice_s - (w_level-(w_width/2)))*(255/(w_width))
...
def load_ct_mask(datasetDir,sub_n, window_specs):
    ...
    CT_scan = np.zeros((ct_scan.shape[0],ct_scan.shape[1],ct_scan.shape[2],3))
    CT_scan[:,:,:,0] = window_ct(ct_scan, 40, 80)  # Convert the CT scans using a brain window
    CT_scan[:,:,:,1] = window_ct(ct_scan, 80, 200) #subdural window
    CT_scan[:,:,:,2] = window_ct(ct_scan,40, 380) #soft tissue window
    # Loading the masks
...
def segment_ct(x, imageLen, windowLen, n_moves, mask = False):
    if mask == False:
        x_segmented = np.zeros([windowLen, windowLen, n_moves * n_moves, 3], dtype=np.uint8)
        counterCrop = 0
...
def prepare_data(dataset_zip_dir, crossvalid_dir, numSubj, imageLen, windowLen, strideLen, n_moves, window_specs):
    ...
    if os.path.isfile(str(Path(crossvalid_dir,'ICH_DataSegmentV1.pkl')))==False: #means the cross-validation folds was created before
        ...
        if os.path.exists(dataset_zip_dir)==True:
            ...
            hemorrhage_diagnosis_array = hemorrhage_diagnosis_df[['PatientNumber','SliceNumber','Intraventricular','Intraparenchymal','Subarachnoid','Epidural', 'Subdural', 'No_Hemorrhage']].to_numpy()   
            ...
            for cvI in range(0,NumCV):
                ...
                for subItrain in range(int(numSubj / NumCV),len(subjectNums_cvI_trainVal)):  # take only the 3 folds for training
                    ...
                    for sliceI in range(0,sliceNums.size):
                        if NoHemorrhage[sliceI] == 0 or sliceI==randomSlice: # Saving only the windows that have hemorrhage or
                            ...
                            x_mask_segment = segment_ct(masks[:, :, sliceI], imageLen, windowLen, n_moves, mask = True)

                            #Saving only the windows that have hemorrhage
                            for i in range(n_moves*n_moves):
                                if sum(sum(x_mask_segment[:, :, i]))>0:
                                    cv2.imwrite(str(Path(crossvalid_dir,'CV' +str(cvI),'train','image',str(counterI) + '.png')), x_ct_segmented[:,:,i,:])
                                    cv2.imwrite(str(Path(crossvalid_dir,'CV' +str(cvI),'train','label',str(counterI) + '.png')), x_mask_segment[:, :,i])
                                    counterI = counterI + 1
                ...
                for subIvalidate in range(0, int(numSubj / NumCV)):  # take only the first fold for validation
                    ...
                    for sliceI in range(0,sliceNums.size):
                        if NoHemorrhage[sliceI] == 0: # Saving only the windows that have hemorrhage
                            ...
                            for i in range(n_moves*n_moves):
                                if sum(sum(x_mask_segment[:, :, i]))>0:
                                    cv2.imwrite(str(Path(crossvalid_dir,'CV' +str(cvI),'validate','image',str(counterI) + '.png')), x_ct_segmented[:,:,i,:])
                                    cv2.imwrite(str(Path(crossvalid_dir,'CV' +str(cvI),'validate','label',str(counterI) + '.png')), x_mask_segment[:, :,i])
                                    counterI = counterI + 1
                ...
                for subItest in range(0, len(subjectNums_cvI_testing)):
                    ...
                    for sliceI in range(0, sliceNums.size):
                        ...
                        cv2.imwrite(str(Path(crossvalid_dir,'CV' + str(cvI),'test','fullCT','image', str(subjectNums_cvI_testing[subItest])
                                          + '_' + str(counterI) + '.png')),ct_scan[:, :, sliceI,:])
                        cv2.imwrite(str(Path(crossvalid_dir,'CV' + str(cvI),'test','fullCT','label', str(subjectNums_cvI_testing[subItest])
                                          + '_' + str(counterI) + '.png')),masks[:, :, sliceI])

                        counterCrop = 0
                        for i in range(n_moves * n_moves):
                            cv2.imwrite(str(Path(crossvalid_dir, 'CV' + str(cvI), 'test','crops', 'image',
                                        str(subjectNums_cvI_testing[subItest])+ '_' + str(counterI) + '_' +
                                        str(counterCrop) + '.png')), x_ct_segmented[:, :, i,:])
                            cv2.imwrite(str(Path(crossvalid_dir, 'CV' + str(cvI), 'test','crops', 'label',
                                        str(subjectNums_cvI_testing[subItest])+ '_' + str(counterI) + '_' +
                                        str(counterCrop) + '.png')), x_mask_segment[:, :, i])
                            counterCrop = counterCrop + 1
            ...
            AllCTscans = np.zeros([hemorrhage_diagnosis_array.shape[0], imageLen, imageLen, 3], dtype=np.uint8)
